...
export const BoothScanner: React.FC<BoothScannerProps> = ({ onClose, onScanSuccess }) => {
    ...
    const handleScanToken = async (attendeeToken: string) => {
        ...
        // 防止重複掃描：如果是同一個 token 且在 5 秒內，則忽略
        const now = Date.now();
        if (lastScannedToken === attendeeToken.trim() && now - lastScanTime < 3000) {
            console.log('忽略重複掃描 - 冷卻時間未到');
            return;
        }

        // 如果正在處理中，直接返回
        if (isProcessingRef.current) {
            console.log('忽略重複掃描 - 正在處理中');
            return;
        }
        ...
        try {
            ...
            const response = await scansServices.scanByToken({
                ...
                booth_token: user.qrCodeToken,
            });

            if (response.success && response.data) {
                const successMsg = `成功記錄！訪客：${response.data.attendee.company || '未知'}${response.data.attendee.name || '未知'}`;
                setResult({
                    success: true,
                    message: successMsg,
                    data: response.data,
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="text-sm text-gray-500 bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-1">
                        ...
                        <p>• 系統將自動記錄訪客資訊</p>
                        <p className="text-purple-600 font-medium mt-2">• 每次掃描後會有 3 秒冷卻時間</p>
                    </div>
                </div>
            </div>
        </div>
