...
interface HourlyStatsProps {
    data: Array<{
        hour: number;  // 改為 number
        scans: number;
        visitors?: number;  // 新增 visitors 欄位（選用）
    }>;
}
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
                <LineChart data={data}>
                    ...
                    <XAxis
                        dataKey="hour"
                        tickFormatter={(hour) => `${hour}:00`}  // 格式化顯示
                    />
                    <YAxis />
                    <Tooltip
                        formatter={(value: number, name: string) => {
                            if (name === 'scans') return [value, '掃描次數'];
                            if (name === 'visitors') return [value, '訪客數'];
                            return [value, name];
                        }}
                    ...
                    />
                    {/* 如果有 visitors 資料也顯示 */}
                    {data.some(d => d.visitors !== undefined) && (
                        <Line
                            type="monotone"
                            dataKey="visitors"
                            stroke="#10B981"
